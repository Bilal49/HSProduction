...
{
    public partial class frmReportCashPayment : Form
    {
        Smartworks.DAL dataAccess = new Smartworks.DAL();
        public frmReportCashPayment()
        {
            InitializeComponent();
        ...
        private void btnViewRepot_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string path = Application.StartupPath + "/rpt/rptCashPayment.rpt";
                document.Load(path);
                DataTable dtReport = new DataTable();
                dtReport = v.GetReportCashPayment("CP", Convert.ToDateTime(dtpFromDate.Text), Convert.ToDateTime(dtpToDate.Text), txtVoucherFromCode.Text, txtVoucherToCode.Text);
                document.SetDataSource(dtReport);
                //document.Subreports[0].SetDataSource(dtReport);
                crystalRptCashPayment.ReportSource = document;
                crystalRptCashPayment.Refresh();
