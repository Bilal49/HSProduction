...
namespace FIL
{
    public partial class frmStockRequisitionApproval : Form
    {
        ...
        public frmStockRequisitionApproval()
        {
            InitializeComponent();
        }
        ...
        private void gvDetail_RowDeleting(object sender, RowDeletingEventArgs e)
        {
            try
            {
                DataRow dr = default(DataRow);
        ...
        private void btnApproved_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(txtRequisitionNo.Text))
            {
                MessageBox.Show("Stock Requisition does not Found.", "Record does not Found", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            DialogResult result = MessageBox.Show("Are you sure want to Approved Stock?", "Requisition Approval", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            switch (result)
            {
                case DialogResult.No:
                    {  
        ...
        private void RequisitionApproval()
        {
            try
            {  
                ...
                if (manageRequisition.UpdateRequisitionApproved(StockReqMasterId, dataAcess))
                {
                    for (int i = 0; i < dset.Tables["REQDetail"].Rows.Count; i++)
                    {
                        DataRow drDetail = dset.Tables["REQDetail"].Rows[i];
                        PM.InsertProductLedger(Convert.ToInt32(drDetail["ProductId"]), Convert.ToDateTime(dtp.Value), txtRequisitionNo.Text, Convert.ToInt32(drDetail["StockReqDetailId"]), "O", Convert.ToDecimal(drDetail["Qty"]), 0, 0, 0, 0, 0, MainForm.User_Id, DateTime.Now.Date, "", "", null, "Out from Stock Requisition" , Convert.ToInt32(cmbFWarehouse.SelectedValue), -1, "" , dataAcess);
                        PM.InsertProductLedger(Convert.ToInt32(drDetail["ProductId"]), Convert.ToDateTime(dtp.Value), txtRequisitionNo.Text, Convert.ToInt32(drDetail["StockReqDetailId"]), "I", Convert.ToDecimal(drDetail["Qty"]), 0, 0, 0, 0, 0, MainForm.User_Id, DateTime.Now.Date, "", "", null, "Inn from Stock Requisition", Convert.ToInt32(cmbTWarehouse.SelectedValue), -1, "", dataAcess);
                    }
                }
                else
                {
                    throw new Exception("Approval Not Update");
                }
                dataAcess.TransCommit();
                lblApproval.Visible = true;
                btnApproved.Enabled = false;
